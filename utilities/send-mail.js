...
exports.notice = (comment) => {
  ...
  let POST_URL =
    process.env.SITE_URL + comment.get("url") + "#" + comment.get("objectId");
...
您可以点击 ${POST_URL} 前去查看！`;
    ...
    request(
      ...
      }?msg=${encodeURIComponent(_template)}`,
      function (error, response, body) {
...
exports.send = (currentComment, parentComment) => {
  ...
  if (MAIL == BLOGGER_EMAIL) {
    _subject =
      process.env.MAIL_SUBJECT_BLOGGER ||
  ...
  let mailOptions = {
    from:
      '"' + process.env.SENDER_NAME + '" <' + process.env.SENDER_EMAIL + ">", // sender address
